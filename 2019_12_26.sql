...
--해당 예외에 대해 다른 로직을 실행시킬수 있게 끔 처리한다.

--예외가 잘행했는데 예외처리가 없는 경우 : PL/SQL블록이 에러와 함꼐 종료된다.
...
--기존에 사용하던 비밀번호 컬럼 이력 관리하기 위한 테이블
...
CREATE OR REPLACE TRIGGER make_history
    ...
    FOR EACH ROW --행 트리거, 행의 변경이 있을 떄 마다 실행한다.
    -- 현재 데이터 참조 : OLD
    -- 갱신 데이터 참조 : NEW
    BEGIN
        -- users 테이블의 pass 컬럼을 변경할떄 trigger 실행
